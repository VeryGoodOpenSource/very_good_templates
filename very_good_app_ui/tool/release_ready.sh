...
previousTag="very_good_app_ui-v${old_version}"
...
echo "Creating git branch for very_good_app_ui@$new_version"
git checkout -b "chore/very_good_app_ui-v$new_version" > /dev/null

git add brick.yaml CHANGELOG.md

echo ""
echo "Run the following command if you wish to commit the changes:"
echo "git commit -m \"chore(very_good_app_ui): v$new_version\""
