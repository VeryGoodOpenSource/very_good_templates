...
{{#flavors}}{{#is_default}}This project contains the following flavors:
...
```sh
{{#flavors}}# {{name.titleCase()}}
$ flutter run --flavor {{name}} --target {{{main}}}

{{/flavors}}```
{{/is_default}}{{/flavors}}{{^flavors}}To run the app either use the launch configuration in VSCode/Android Studio or use the following command:
