...
previousTag="very_good_flutter_plugin-v${old_version}"
...
if [ -f "hooks/lib/version.dart" ]; then
  sed -i '' "s/\$templateVersion = '$old_version'/\$templateVersion = '$new_version'/g" hooks/lib/version.dart
fi
...
echo "Creating git branch for very_good_flutter_plugin@$new_version"
git checkout -b "chore/very_good_flutter_plugin-v$new_version" > /dev/null

git add brick.yaml CHANGELOG.md hooks/lib/version.dart

echo ""
echo "Run the following command if you wish to commit the changes:"
echo "git commit -m \"chore(very_good_flutter_plugin): v$new_version\""